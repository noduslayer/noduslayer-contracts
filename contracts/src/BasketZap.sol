...
///      backing, the caller's minimum output and the deadline. It holds no balances between transactions:
///      every leftover is refunded to the caller, so a malformed route can only cost its own sender.
contract BasketZap is IBasketZap, Ownable2Step, Pausable, ReentrancyGuard {
    ...
    function zapMint(
        ...
        IBasketToken.Constituent[] memory recipe = _stageConstituents(basket, shares);
        netShares = IBasketToken(basket).mint(shares, to);

        for (uint256 i; i < recipe.length; ++i) {
            _refund(recipe[i].token);
        }
        _refund(tokenIn);
    ...
    function zapRedeem(
        ...
        uint256 gross = IERC20(tokenOut).balanceOf(address(this));
    ...
    function _execute(Swap[] calldata swaps) private {
        address openToken;
        address openRouter;
        ...
        for (uint256 i; i < swaps.length; ++i) {
            ...
            if (leg.sellToken != openToken || leg.router != openRouter) {
                ...
                IERC20(openToken).forceApprove(openRouter, IERC20(openToken).balanceOf(address(this)));
            }
            if (leg.prefund != 0) IERC20(leg.sellToken).safeTransfer(leg.router, leg.prefund);
    ...
    /// @dev Verifies the swaps produced enough of every constituent and makes it spendable by the basket.
    ///      The allowance is left at max because it is only reachable while this contract is `msg.sender`.
    function _stageConstituents(address basket, uint256 shares)
        private
        returns (IBasketToken.Constituent[] memory recipe)
    {
        recipe = IBasketToken(basket).constituents();
